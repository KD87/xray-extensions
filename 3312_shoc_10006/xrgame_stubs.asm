...
; глобальная переделка системы FOV оружия: увеличение ствола задаётся не абсолютным значением FOV,
; а множителем, на который умножается текущий глобальный FOV
; по идее снимает все проблемы с настройкой и перенастройкой стволов для разных FOV, а также
...
org 10234388h - shift
	jmp CWeaponBinoculars__ZoomInc_fix ; обратно не возврашаемся, там вся функция целиком
...
org 10225B47h - shift
loc_10225B47:
; CWeaponMagazined::switch2_Empty()
org 1022707Dh - shift
	jmp CWeaponMagazined__switch2_Empty_fix
org 10225B70h - shift
...
loc_10227093:
; =========================================================================================
; отключение анимации anim_playing для болта и гранат
; CMissile::UpdateCL()
org 10223070h - shift
	jmp loc_10223090
	nop
...
;Вариант 1: расчёт по очкам (рекомендуется)
org 101AE902h - shift	; 3 bytes
	mov [eax+10h], ecx

;Вариант 2: расчёт по штукам
org 101AE8FDh - shift	; 8 bytes
	dword	90909090h
	nop
	mov [eax+10h], ecx

;С этой врезкой есть косяк - данная константа используется также и в инвентаре для обозначения стопок предметов (х5 например)
;По этой причине второй вариант использовать не рекомендуется. Тем более, изначальная идея GSC была в подсчете именно по очкам.
...
; -----
; Конфликтует с фиксом PDA_KILLS_STATISTIC_FIX_VER_1 от kstn и по большому счету не исправляет проблему.
; Исправляет только счетчик секции (например "Убийство мутантов"), при этом детальное описание и общий счетчик "Всего" продолжает показывать туфту.
; Фикс от kstn правит все эти моменты, поэтому использовать данную правку не рекомендуется.
org 10203430h - shift
	nop
	nop
	nop
